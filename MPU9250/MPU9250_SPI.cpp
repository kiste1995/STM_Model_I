...
{
  Vect3 _a, _g, _m;
  _a.x= a.y; _a.y= a.x; _a.z= -a.z; // why???
...
void MPU9250_SPI::magCalMPU9250()
{
    ...
    for(ii = 0; ii < sample_count; ii++)   {
        ...
        if(_mMode == MGN_CONT_MEAS1) ThisThread::sleep_for(135);  // at 8 Hz ODR, new mag data is available every 125 ms
        if(_mMode == MGN_CONT_MEAS2) ThisThread::sleep_for(12);  // at 100 Hz ODR, new mag data is available every 10 ms
    }
#if (NO_ROS)
...
void MPU9250_SPI::readAK8963Bytes(uint8_t subAddress, uint8_t count, uint8_t* dest)
{
   ...
   ThisThread::sleep_for(1); // takes some time for these registers to fill
...
uint8_t MPU9250_SPI::readAK8963Byte(uint8_t subAddress)
{
  ...
  ThisThread::sleep_for(11); // takes some time for these registers to fill
